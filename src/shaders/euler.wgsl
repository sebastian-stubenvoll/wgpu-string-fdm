...
fn fdm_step(index: u32, future: u32, current: u32) {
    ...
    if (index > 1u && index < uniforms.node_count - 2u) {
        ...
        let neighbour = u32(index != 0) * (uniforms.node_count - 3) + 1;

        nodes[index].positions[future] = 0.0;
        nodes[index].positions[future + 1] = nodes[1 + neighbour].positions[future + 1];
        nodes[index].velocities[future] = 0.0;
        nodes[index].velocities[future + 1] = nodes[1 + neighbour].velocities[future + 1];
    }
}
